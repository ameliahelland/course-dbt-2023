...
WITH ProductOrderCounts AS (
    SELECT
        ...
        COUNT(*) AS Frequency
    FROM
        {{ ref('postgres.stg_postgres__order_items') }} oi
    JOIN
        {{ ref('postgres.stg_postgres__products') }} p ON oi.product_id = p.product_id
    JOIN
        {{ ref('postgres.stg_postgres__order_items') }} op ON oi.order_id = op.order_id
    WHERE
...
JOIN
    {{ ref('postgres.stg_postgres__products') }} p ON r.RelatedProductID = p.product_id
WHERE
    r.RowNum = 1
ORDER BY
    r.Frequency DESC